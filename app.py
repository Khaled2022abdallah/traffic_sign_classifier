...
# Load the TensorFlow model from th
model = tf.keras.models.load_model('static/saved_model/traffic_sign_classifier.keras')
...
def preprocess_image(image):
    """Preprocess the uploaded image to match the model input requirements.txt."""
    # Resize the image to 32x32
    image = image.resize((32, 32))
    # Convert the image to grayscale
    image = image.convert('L')
    # Convert the image to a numpy array convert
    image_array = np.array(image)
    # Normalize the image data
    image_array = image_array / 128
